...
class BreakoutR(nn.Module):
    def __init__(self,in_channel,outputs):
        ...
        self.conv2 = nn.Conv2d(in_channels = 7, out_channels = 5, kernel_size = 4, stride = 2)
        self.max1 = nn.MaxPool2d(kernel_size = 3)
        self.conv3 = nn.Conv2d(in_channels = 5, out_channels = outputs, kernel_size = 5, stride = 3)
...
def optimize_model(obs_list, act_list, rew_list):
    ...
    #print(rew_list)
    prb_list = torch.gather(policy_net(obs_list),1,act_list)
    L = torch.mean(-torch.log(prb_list)*rew_list)
...
filename='Breakout_Reinforcement'
...
while i_episode <= max_num_episodes:
    ...
    for t in count():
        ...
        if not done:
            ...
            obs_window.append(next_observation)
    ...
    test_reward = '-'
    if i_episode % 50 == 0 and i_episode != 0:
        model_name='ReinforcementBreakout'+str(i_episode)+'.pth'
