...
parser.add_argument('--game', default='BreakoutDeterministic-v4', help='OpenAI gym environment name', dest='game', type=str)
...
def learn_proc(mem_queue, weight_dict):
    ...
    if checkpoint > 0:
        ...
        agent.train_net.load_weights(os.path.join(rootdir, "saved_weights", 'model-%s-%d.h5' % (args.game, checkpoint,)))
        # -----
    ...
    while True:
        ...
        if save_counter < 0:
            ...
            agent.train_net.save_weights(os.path.join(saveWeightFolder, 'model-%s-%d.h5' % (args.game, agent.counter,)), overwrite=True)
...
def generate_experience_proc(mem_queue, weight_dict, no):
    ...
    while True:
        ...
        msg = ' %5d> Reward: %4d; Average: %6.2f; Max: %4d' % (episode_count, episode_reward, np.mean(avg_score), np.max(avg_score))
...
def saveCSV(name, rewards_list):
    ...
    df.to_csv(csvPath)
# def checkLRP(model):
#     imgDir = os.path.join(rootdir, "LRP_Analysis")
#
#     typeAnalysis = []
#     for i in ["gradient", "guided_backprop", "pattern.attribution", "integrated_gradients", "lrp.z"]:
#         analyzer = innvestigate.create_analyzer(i, model)
#         typeAnalysis.append(analyzer)
#
#     for i in typeAnalysis:
#         a = i.analyzer(observation)
...
#         #save
#         AnalysisImgsPath = os.path.join(imgDir, "Episode " + str(i_episode) + i + ".PNG")
#         plt.savefig(AnalysisImgsPath)
...
def renderRewardGraph(name, rewardList):
    ...
    imgPath = os.path.join(rootdir, str(name) + "Thread_rewardVSEps_Episode" + str(len(rewardList)) +".PNG")
    plt.savefig(imgPath)
