...
def learn_proc(mem_queue, weight_dict):
    ...
    agent = LearningAgent(env.action_space, batch_size = arguments["BatchSize"], swap_freq=arguments["SwapRate"])
...
class ActingAgent(object):
    ...
    # def choose_epsilon(self): #TODO make eps for choose_action
...
def generate_experience_proc(mem_queue, weight_dict, no):
    ...
    env = gym.make('BreakoutDeterministic-v4')
    agent = ActingAgent(env.action_space, n_step=arguments["N-Steps"])
...
def main():
    ...
    mem_queue = manager.Queue(arguments["ExperienceQueueSize"])
